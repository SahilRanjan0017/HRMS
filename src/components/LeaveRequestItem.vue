	...
	>
		<template #left>
			...
			<div class="flex flex-col items-start gap-1">
				...
				<div
					class="text-xs font-medium text-[rgb(var(--text-muted))] flex items-center gap-1.5"
				>
					<span>{{ props.doc.leave_dates || getLeaveDates(props.doc) }}</span>
					<span
						class="w-1 h-1 rounded-full bg-[rgb(var(--border-color))]"
					></span>
					<span class="font-bold text-blue-500">{{
						__("{0}d", [props.doc.total_leave_days])
					}}</span>
				</div>
			</div>
		</template>
