...
export const AttendanceCalendar: React.FC<AttendanceCalendarProps> = ({
	...
	for (let i = 1; i <= endDate.date(); i++) {
		days.push(dayjs(`${month.year()}-${String(month.month() + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`))
	}
	...
	return (
		<div className="p-4">
			<div className="flex items-center justify-between mb-6">
				<h3 className="text-lg font-bold text-gray-900">{month.format('MMMM YYYY')}</h3>
				...
				<thead>
					<tr>
						{['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day) => (
							<th key={day} className="text-center py-2 text-xs font-semibold text-gray-600 bg-gray-50">
								{day}
							</th>
						))}
					</tr>
							...
							{days.slice(weekIdx * 7, weekIdx * 7 + 7).map((date, dayIdx) => {
								...
								return (
									<td
										...
										onClick={() => date && onDateClick && onDateClick(date.format('YYYY-MM-DD'))}
									>
										{date ? (
											<button
